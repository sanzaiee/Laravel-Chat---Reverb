<div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            {{-- chat container --}}
            <div id="chat-container" class="bg-white shadow-sm sm:rounded-lg mb-15 overflow-hidden h-cal[(100vh-12rem)] scroll-smooth">
                                    ...
                                    <div class="grid pb-2">
                                        ...
                                        <div class="w-max grid">
                                            @if($message->message)
                                               ...
                                               @else
                                                {{-- if file type is imag --}}
                                                <a download href="{{ asset($message->folder_path) }}">
                                                    @if (str_starts_with($message->file_type, 'image/'))
                                                        <img class="w-30 h-20 object shadow-md-cover" src="{{ asset('storage/'.$message->folder_path) }}" />
                                                    @else
                                                    <div class="flex items-center justify-between bg-grey-100 gap-2.5 justify-end pb-4">
                                        ...
                                        {{-- if file type is image --}}
                                            <a download href="{{ asset($message->folder_path) }}">
                                                @if (str_starts_with($message->file_type, 'image/'))
                                                    <img class="w-30 h-20 object shadow-md-cover" src="{{ asset('storage/'.$message->folder_path) }}" />
                                                @else
                                                <div class="flex items-center justify-between bg-indigo-600 gap-2.5 justify-end pb-4">
                                                    <svg class="cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="22" height="22"
                                                     viewBox="0 0 22 22" fill="none">
                                                        <g id="Attach 01">
                                                        <g id="Vector">
                        ...
                        <div class="w-full pl-3 pr-1 py-1 rounded-3xl border border-gray-200 items-center gap-2 inline-flex justify-between">
                            ...
                            @if($file)
                                ...
                                @if($isImage)
                                    ...
                                    class="w-12 h-12 object-cover rounded-lg border border-gray-300 shadow-md" alt="">
                                @else
                                <span class="w-full max-w-64">
...
<script type="module">
    ...
    window.Echo.private(`chat-channel.{{ $sender_id }}`).listen('UserTyping',(event)=>{
